...
# --- 10. CMAKE CONFIGURE ---
Log "CMake Konfiguration (CUDA, TurboQuant+)"
...
# Hinweis: TurboQuant-Typen (TQ3_1S/TQ4_1S/turbo2/turbo3/turbo4) werden automatisch
# verfuegbar, sobald das passende Backend (hier CUDA) einkompiliert ist. Keine
# zusaetzlichen CMake-Flags noetig.
$cmakeArgs = @(
    "-S", $dir, "-B", $buildDir,
    "-G", $vsGenerator, "-A", "x64",
...
# --- 11. BUILD ---
Log "Kompiliere llama-cpp-turboquant mit $PARALLEL_JOBS Jobs..."
...
Write-Host "`nServer starten (Beispiel mit asymmetrischer KV-Quantisierung):" -ForegroundColor Green
Write-Host "  $binPath\llama-server.exe -m <model.gguf> --cache-type-k q8_0 --cache-type-v turbo3 --host 0.0.0.0 --port 8080" -ForegroundColor Green
